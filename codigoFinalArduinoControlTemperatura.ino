...
void comunicacion() {
    // Verifica si hay datos disponibles en Serial2
    if (Serial2.available() > 0) {
        // Leer la cadena de texto completa desde Serial2
        ...
        const size_t capacity = JSON_ARRAY_SIZE(1) + JSON_OBJECT_SIZE(4) + 60;  // Ajusta según el tamaño de tu JSON
        ...
        // Verificar si el JSON es un arreglo y obtener el primer objeto
        if (doc.is<JsonArray>()) {
            JsonArray array = doc.as<JsonArray>();
            JsonObject obj = array[0];
            
            // Extraer los valores de temperature y state
            ...
            temperaturaSetpoint = setpoint(temperature);

            // Aquí puedes usar los valores de temperature y state para el control de tu proceso
        } else {
            Serial.println("El JSON no es un arreglo");
        }
    }
}
...
float setpoint (String temperature) {
...
void controlTemperatura1() {
    ...
    Serial.println(temperaturaMedida1);
